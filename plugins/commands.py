...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
            ...
            InlineKeyboardButton('✪ Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url="https://t.me/+QENrEno72xQwOTg1"),
            InlineKeyboardButton('⌬ Mᴏᴠɪᴇ Gʀᴏᴜᴘ', url='https://t.me/+QENrEno72xQwOTg1')
        ],[
    ...
    if len(message.command) != 2:
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        m=await message.reply_sticker("CAACAgIAAxkBAAJmGmY4QdSV9HUWG5rFF46wB6FEUYHrAAJOAgACVp29CjD-a22BMgNvHgQ") 
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        await client.send_photo(
            ...
            photo="https://graph.org/file/172eaa72340bfdc8f669d.jpg",
            caption="**You are not in our channel given below so you don't get the Episode...\n\nIf you want Episode, click on the 'Join Update channel 🔥' button below and join our back-up channel, then click on the '🔄 Try Again' button below...\n\nThen you will get Episodes...**",
    ...
    if data.split("-", 1)[0] == "Sandip":
        ...
        if vj and PREMIUM_AND_REFERAL_MODE == True:
            ...
            reply_markup = InlineKeyboardMarkup(buttons)
            m=await message.reply_sticker("CAACAgEAAxkBAAKeRGZpC0oLRAf-IeeCEzdqcTthW1oVAALIAANN0ZBHymniXLabHuweBA") 
            await asyncio.sleep(1)
            await m.delete()
            await message.reply_photo(
                photo=random.choice(PICS),
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            try:
                ...
                if STREAM_MODE == True:
                    button = [[
                       InlineKeyboardButton('Developer', url="https://t.me/Sandip10x")
                    ]]
                else:
                    button = [[
                       InlineKeyboardButton('Developer', url="https://t.me/Sandip10x")
                ...
                filesarr.append(msg)
                k = await client.send_message(chat_id = message.from_user.id, text=f"<b>File Will Delete In 10 Minutes Please Forward It !! 💊 </b>")
    ...
    if data.startswith("sendfiles"):
        ...
        k = await client.send_message(chat_id=message.from_user.id,text=f"<b>Get All Files in a Single Click!!!</b>\n\n📂 Lɪɴᴋ ➠ : {g}\n\n<b>Note:</b> This message is deleted in 5 mins to avoid copyrights. Save the link to Somewhere else", reply_markup=InlineKeyboardMarkup(
                [
                    [
        ...
        for file in files:
            ...
            filesarr.append(msg)
        k = await client.send_message(chat_id = message.from_user.id, text=f"<b>File Will Delete In 10 Min Please Forward It !! 💊</b>")
        await asyncio.sleep(600)
        for x in filesarr:
            await x.delete()
        await k.edit_text("<b>Your All Files/Videos is successfully deleted!!!</b>")
    ...
    files = files_[0]
    title = '⚜️ (@MythoSerialBot) -' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"⚜️ (@MythoSerialBot) - {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await db.has_premium_access(message.from_user.id):
        if not await check_verification(client, message.from_user.id) and VERIFY == True:
            btn = [[
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
    ...
    if len(message.command) == 3:
        ...
        if seconds > 0:
            ...
            await client.send_message(
                chat_id=user_id,
                text=f"<b>Premium Added To Your Account {time} Enjoy !! Check /myplan 😀\n</b>",                
            )
        else:
...
async def plans_cmd_handler(client, message): 
    ...
    btn = [            
        ...
        [InlineKeyboardButton("⚠️ Close It ⚠️", callback_data="close_data")]
    ]
    reply_markup = InlineKeyboardMarkup(btn)
    await message.reply_photo(
        photo=PAYMENT_QR,
